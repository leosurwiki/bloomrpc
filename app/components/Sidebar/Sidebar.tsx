        ...
        <div style={{ width: "50%" }}>
          ...
          >
            ...
            >
              <Icon
                type="reload"
                style={{ cursor: "pointer", color: "#1d93e6" }}
              />
            </Button>
          </Tooltip>
        ...
        {protos.length > 0 && (
          <Tree.DirectoryTree
            ...
            onSelect={async selectedKeys => {
              ...
              onMethodSelected(
                protoDefinitions.methodName,
                protoDefinitions.protodef.services[protoDefinitions.serviceName]
              );
            }}
            onDoubleClick={async (event, treeNode) => {
              const selected = treeNode.props.eventKey;
              if (selected && selected.includes("message:")) {
                const fragments = selected.split("||");
                const fileName = fragments[0];
                const messageName = fragments[1].replace("message:", "");
                const protodef = protos.find(protoFile => {
                  return (
                    protoFile.fileName.endsWith(fileName) ||
                    Boolean(protoFile.messages[messageName])
                  );
                });
                if (protodef) {
                  const protoMessage = protodef.messages[messageName];
                  SideBarStore.publishMessageClicked(
                    `\"type_url\":\"${messageName}\",\n` +
                      `\"content\":${JSON.stringify(protoMessage.plain)}\n`
                  );
                }
              } else {
                const protoDefinitions = processSelectedKey(selected);
                ...
                onMethodDoubleClick(
                  protoDefinitions.methodName,
                  protoDefinitions.protodef.services[
          ...
          >
            {protos.map(proto => (
              ...
              >
                  ...
                  >
                    {proto.services[service].methodsName
                      .filter(name => {
                        ...
                        return name
                          .toLowerCase()
                          .includes(filterMatch.toLowerCase());
                      })
                      .map((method: any) => (
                        <Tree.TreeNode
                          icon={<Badge type="method"> R </Badge>}
                          title={method}
                          key={`${
                ...
                {Object.keys(proto.messages).filter(m => !m.startsWith("google")).map(message => (
                  <Tree.TreeNode
                    icon={<Badge type="message"> M </Badge>}
                    title={message}
                    key={`${proto.fileName}||message:${message}`}
                  />
                ))}
              </Tree.TreeNode>
            ))}
